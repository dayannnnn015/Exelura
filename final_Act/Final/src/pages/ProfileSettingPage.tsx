...
import { Close, Edit, PhotoCamera, Verified, Logout, Settings } from "@mui/icons-material";
...
const ProfileSettingPage: React.FC = () => {
  ...
  return (
    <Box sx={{ minHeight: "100vh", backgroundColor: "#0A081F", color: "#fff", pb: 4 }}>
      {/* Header */}
      ...
      <Box sx={{ px: 3, pt: 3 }}>
        ...
        {activeTab === 1 && (
          <Paper elevation={0} sx={{ p: 3, bgcolor: alpha('#FFFFFF', 0.05), borderRadius: 2 }}>
            ...
            {/* Add password fields and 2FA here */}
          </Paper>
        )}
        ...
        {activeTab === 2 && (
          <Paper elevation={0} sx={{ p: 3, bgcolor: alpha('#FFFFFF', 0.05), borderRadius: 2 }}>
            ...
            {/* Add payment cards here */}
          </Paper>
        )}

        <Box sx={{ mt: 4, display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
          <Button onClick={handleSave} variant="contained" sx={{ background: 'linear-gradient(135deg, #7877C6 0%, #5A59A1 100%)' }}>Save Changes</Button>
        </Box>
      </Box>
    </Box>
  );
